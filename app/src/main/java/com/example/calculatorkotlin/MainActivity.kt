...
class MainActivity : AppCompatActivity() {
    private val totalNumbers = mutableListOf<Int>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btAvg.setOnClickListener { doSomething("avg") }
        btMed.setOnClickListener { doSomething("med") }
    }
    ...
    private fun doSomething(p0: String) {
        ...
        if (tvExpression.text.toString().isEmpty()) {
            ...
            when (p0) {
                ...
                "avg" -> {
                    val allNumbers = fetchNumbers(tvExpression.text.toString())
                    tvExpression.text = allNumbers.average().toString()
                    tvResult.text = ""
                }
                "med" -> {
                    val allNumbers = fetchNumbers(tvExpression.text.toString())
                    tvExpression.text = med(allNumbers).toString()
                    tvResult.text = ""
                }
            }
        }
    ...
    private fun fetchNumbers(p0: String): List<Double> {
        val list = p0.split('+', '-', '×', '÷', '%', ignoreCase = true).mapNotNull {
            it.toDoubleOrNull()
        }
        return list
    }
